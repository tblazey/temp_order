...
#Setup the positions list and then get one in a random order
    ...
    if len(event.getKeys(keyList=['f'])) > 0:
        ...
        for object,box in zip(objects,boxes):
            ...
            if (object.pos[0] == start_dictionary[object.name][0]  and object.pos[1] == start_dictionary[object.name][1] ):
                in_orig += 1
        confirm = visual.TextStim(window,text='There are ' + str(in_orig) + ' left in their original places. Press f to quit or b to go back')
        confirm.draw()
        window.flip()
        while True:
            if len(event.getKeys(keyList=['f'])) > 0:
